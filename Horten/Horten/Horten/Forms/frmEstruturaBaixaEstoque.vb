...
Public Class frmEstruturaBaixaEstoque
    ...
    Public Sub New(ByVal ssProduto As String)
        InitializeComponent()
        sProduto = ssProduto
        If sProduto <> "" Then
            txtCDProduto.Text = sProduto
            buscar(sProduto)
        End If
    End Sub
    ...
    Private Sub bt0Buscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bt0Buscar.Click
        ...
        buscar(txtCDProduto.Text)
    End Sub
    Public Sub buscar(ByVal ssProduto As String)
        conSQL2 = conn.sqlConnect(gDataSource, gUserID, gPWD, gInitialCatalog)
        conSQL2.Open()
        querySQL2.Connection = conSQL2

        querySQL2.CommandText = "Select * From Estoque Where CDProduto = '" & ssProduto.PadLeft(7, "0") & "'"
    ...
    Public Sub carregaGrid()
        ...
        adaptSQL = New SqlClient.SqlDataAdapter("Select * From EstruturaEstoque  Where CDProduto = '" & sProduto.PadLeft(7, "0") & "' order by CDMaterial", conSQL2)
        adaptSQL.Fill(datPubsEstoque, "EstruturaMP")
        GridControl1.DataSource = datPubsEstoque.Tables("EstruturaMP")
    End Sub
    Private Sub bt0Imprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles bt0Imprimir.Click
        If sProduto <> "" Then
            ...
            End If
            HeaderArea.Content.Add("Estrutura de Estoque: " & sProduto & " - " & sNomeProd)
    ...
    Private Sub GridControl1_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles GridControl1.KeyUp
        If e.KeyCode = Keys.Delete Then
            ...
            If classParam.selPermissoes(296, gCDPerfil, gDataSource, gUserID, gPWD, gInitialCatalog) = True Then
                ...
                querySQL2.Connection = conSQL2
                Try
                    querySQL2.CommandText = "delete from EstruturaEstoque where CDProduto='" & txtCDProduto.Text & "' and CDMaterial ='" & row("CDMaterial") & "'"
        ...
        Try
            If row.RowState = DataRowState.Added Then
                If classParam.selPermissoes(294, gCDPerfil, gDataSource, gUserID, gPWD, gInitialCatalog) = True Then
                    ...
                    querySQL2.Connection = conSQL2
                    Try
                        querySQL2.CommandText = "Insert into EstruturaEstoque (CDProduto,CDMaterial,Qtde,Unidade,Descricao) values ('" & txtCDProduto.Text & "','" & row("CDMaterial") & "'," & Replace(row("Qtde"), ",", ".") & ",'" & row("Unidade") & "','" & row("Descricao") & "')"
                        querySQL2.ExecuteNonQuery()
                        carregaGrid()
                    Catch ex As Exception
                        MessageBox.Show(ex.Message(), "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub GridView1_RowUpdated(ByVal sender As Object, ByVal e As DevExpress.XtraGrid.Views.Base.RowObjectEventArgs) Handles GridView1.RowUpdated
        ...
        If row.RowState = DataRowState.Modified Then
            If classParam.selPermissoes(295, gCDPerfil, gDataSource, gUserID, gPWD, gInitialCatalog) = True Then
                ...
                querySQL2.Connection = conSQL2
                Try
                    querySQL2.CommandText = "update EstruturaEstoque set Qtde=" & Replace(row("Qtde"), ",", ".") & ",Unidade='" & row("Unidade") & "' where CDProduto='" & txtCDProduto.Text & "' and CDMaterial ='" & row("CDMaterial") & "'"
                    querySQL2.ExecuteNonQuery()
                Catch ex As Exception
                    MessageBox.Show(ex.Message(), "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error)
                End Try
    ...
    Private Sub SimpleButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SimpleButton1.Click
        Dim row As System.Data.DataRow = GridView1.GetDataRow(GridView1.FocusedRowHandle)
        Dim sCDItem As String
        Try
            sCDItem = row("CDMaterial")
        Catch ex As Exception
            sCDItem = ""
        End Try
        If sCDItem <> "" Then
            Dim s As frmEstruturaBaixaMP = New frmEstruturaBaixaMP(sCDItem)
            s.Show()
        Else
            MessageBox.Show("Selecione um item de estoque!", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End If
    End Sub
            ...
            If datRead.Read = True Then
                ...
                If datRead.Read = True Then
                    ...
                    If MessageBox.Show("Confirma a cópia da estrutra?" & vbCrLf & "Lembre-se que a estrutura filho será apagada e recriada de acordo com a estrutura pai!", "Pergunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
                        ...
                        querySQL2.CommandText = "delete from EstruturaEstoque where CDProduto='" & VCodFilho.PadLeft(7, "0") & "'"
                        querySQL2.ExecuteNonQuery()

                        '#PEGA DADOS DA ESTRUTURA PAI
                        querySQL2.CommandText = "Select * from EstruturaEstoque where CDProduto='" & VCodPai.PadLeft(7, "0") & "'"
                        datRead2 = querySQL2.ExecuteReader()
                        Do Until datRead2.Read = False
                            'JOGA DADOS NA ESTRUTURA FILHA
                            querySQl.CommandText = "Insert into EstruturaEstoque (CDProduto,CDMaterial,Qtde,Unidade, Descricao) values ('" & VCodFilho.PadLeft(7, "0") & "','" & datRead2("CDMaterial") & "'," & Replace(datRead2("Qtde"), ",", ".") & ",'" & datRead2("Unidade") & "','" & datRead2("Descricao") & "')"
                            querySQl.ExecuteNonQuery()
                        Loop
                        datRead2.Close()
                        MessageBox.Show("Estrutura copiada com sucesso!", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
                    End If
                Else
                    datRead.Close()
